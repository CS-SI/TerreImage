...
class QGISEducationWidget(QtGui.QWidget, Ui_QGISEducation, QtCore.QObject):
    ...
    def setupUi_extra(self):
        ...
        for index in range(len(itemProcessing)):
            ...
            if index > 0:
                action_p = QtGui.QAction(
                  QtGui.QIcon(":/plugins/qgiseducation/img/mAction.png"),
                  item,
        ...
        action_ps = QtGui.QAction(
          QtGui.QIcon(":/plugins/qgiseducation/img/mIconTableLayer.png"),
          "Profil Spectral",
          self.iface.mainWindow())
        ...
        action_classif_ns = QtGui.QAction(
        QtGui.QIcon(":/plugins/qgiseducation/img/rendererCategorizedSymbol.png"),
        u"Classification non supervisée",
        self.iface.mainWindow())
        action_classif_ns.setWhatsThis(u"Classification non supervisée")
        m4.addAction(action_classif_ns)
        action_classif_s = QtGui.QAction(
        QtGui.QIcon(":/plugins/qgiseducation/img/rendererCategorizedSymbol.png"),
        u"Classification supervisée",
        self.iface.mainWindow())
        ...
        action_kmz = QtGui.QAction(
          QtGui.QIcon(":/plugins/qgiseducation/img/mMapserverExport.png"),
          "Export KMZ",
          self.iface.mainWindow())
        ...
        action_info = QtGui.QAction(
          QtGui.QIcon(":/plugins/qgiseducation/img/mActionContextHelp.png"),
          "Information",
          self.iface.mainWindow())
...
#         manage_QGIS.custom_stretch(ProcessingManager().working_layer.qgis_layer, values, self.canvas)
                ...
                for i in range(len(process)):
                    ...
                    if i > 0:
                        action_h = QtGui.QAction(
                          ...
                          process[i],
                          self.iface.mainWindow())
                        action_h.setWhatsThis(process[i])
                        m2.addAction(action_h)
                    if i == 1:
                        self.toolButton_histograms.setDefaultAction(action_h)
            ...
            for key in corres:
                if corres[key] == text_changed :
                    ...
                    if corres_name_view[who] in ProcessingManager().get_processings_name() :
                        do_it = False
                    # band_to_display = ProcessingManager().working_layer.bands[key]
                    # manage_QGIS.display_one_band(ProcessingManager().working_layer, who, self.iface)
                    if do_it :
    ...
    def kmeans(self):
        ...
        if ProcessingManager().working_layer == None :
            ...
            self.qgis_education_manager.value_tool.set_layers(ProcessingManager().get_working_layers())
            # terre_image_processing.kmeans(ProcessingManager().working_layer, self.qgis_education_manager.working_directory, self.iface, nb_class)
            self.set_working_message(False)
